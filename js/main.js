// navbar styles
const openHamburger = document.querySelector(".fa-solid");
const active = document.querySelector(".active");
const closeHamburger = document.querySelector(".fa-regular");
    ...
    .then((data) => {
      ...
      for (x in data[0].meanings) {
        for (y in data[0].meanings[x].definitions) {
          if (
            "definition" in data[0].meanings[x].definitions[y] &&
            "example" in data[0].meanings[x].definitions[y]
          ) {
...
function myToggle() {
  if (
    rs.getPropertyValue("--button-color") ==
    rs.getPropertyValue("--bt-light-mode")
  ) {
    r.style.setProperty(
      "--primary-color",
      rs.getPropertyValue("--p-dark-mode")
    );
    r.style.setProperty(
      "--secondary-color",
      rs.getPropertyValue("--s-dark-mode")
    );
    ...
    r.style.setProperty(
      "--button-color",
      rs.getPropertyValue("--bt-dark-mode")
    );
    r.style.setProperty(
      "--button-color-alt",
      rs.getPropertyValue("--bt-alt-dark-mode")
    );
  } else if (
    rs.getPropertyValue("--button-color") ==
    rs.getPropertyValue("--bt-dark-mode")
    ...
    r.style.setProperty(
      "--primary-color",
      rs.getPropertyValue("--p-light-mode")
    );
    r.style.setProperty(
      "--secondary-color",
      rs.getPropertyValue("--s-light-mode")
    );
    r.style.setProperty("--text-color", rs.getPropertyValue("--t-light-mode"));
    r.style.setProperty(
      "--indent-color",
      rs.getPropertyValue("--i-light-mode")
    );
    ...
    r.style.setProperty(
      "--button-color-alt",
      rs.getPropertyValue("--bt-alt-light-mode")
    );
  }
}
